...
--
--
--data BkTree a = BKNode a (SparseArray (BkTree a)) | BKLeaf a | BKNil deriving Eq
--
...
--}
--
--newtype SparseArray a = SparseArray [(Int,a)] deriving Eq
--
--aindex :: (Eq a) => SparseArray a -> a -> Int
--aindex [] _ = -1
--aindex (x:xs) a = if snd x == a then fst a  else aindex xs a
--
--(!!) :: SparseArray a -> Int -> a
--[] !! i = undefined
--(x:xs) !! i = if fst a == i then snd x else if fst a > i then undefined else xs ! (//)
--
--(///) :: SparseArray a -> [(Int,a)] -> SparseArray a
--r///l = foldl' (ainsert) (SparseArray []) l
--
--ainsert :: SparseArray a -> (Int,a) -> SparseArray a
--ainsert [] v = [v]
--ainsert (x:xs) v = if fst a == fst v then a:xs else if fst a > fst v then v:x:xs else x:(ainsert xs v)
--
--
